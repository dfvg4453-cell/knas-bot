...
def start_command(message):
    ...
    # معالجة نظام رابط الإحالة (تجميع النقاط)
    ...
    if user_id not in users:
        ...
        # مكافأة صاحب رابط الإحالة (0.01 نقطة)
        if referrer_id and referrer_id in users and referrer_id != user_id:
            users[referrer_id]["balance"] += 0.01
            try:
                bot.send_message(
                    int(referrer_id), 
                    f"🎁 **تم إضافة 0.01 نقطة لفريد حسابك!**\nقام شخص بدخول البوت عبر رابط الدعوة الخاص بك.",
                    parse_mode='Markdown'
                )
            except Exception:
                pass
    ...
    balance = round(users[user_id]["balance"], 4)
    welcome_text = (
...
# زر تجميع النقاط والإحالة
@bot.message_handler(func=lambda msg: msg.text == '🎁 مشاركة وتجميع رصيد')
def share_and_earn(message):
    ...
    text = (
        ...
        f"`{referral_link}`\n\n"
        f"قم بنشر الرابط، وكل شخص يدخل البوت عبر رابطك ويفعّله ستحصل فوراً على **0.01 نقطة/دولار** في رصيدك!"
    )
...
# زر شحن الرصيد
@bot.message_handler(func=lambda msg: msg.text == '💳 شحن الرصيد')
def charge_balance_info(message):
    charge_text = (
        "أهلاً بك 💳\n\n"
    ...
    bot.reply_to(message, charge_text, disable_web_page_preview=True)
...
def show_stats(message):
    if message.from_user.id == ADMIN_ID:
        ...
        users = load_data(USERS_FILE)
        user_balance = round(users.get(str(message.from_user.id), {}).get("balance", 0.0), 4)
...
def admin_panel(message):
    ...
    panel_text = (
        ...
        "• `/admin` - فتح لوحة تحكم الأدمن الرئيسية للوصول السريع إلى كافة الإعدادات.\n"
        "• `/stats` - عرض إحصائيات البوت الشاملة (عدد المستخدمين الكلي، وإجمالي النقاط المُوزعة).\n"
        "• `/add_points <id> <points>` - إضافة رصيد لحساب مستخدم معين (تتطلب إرسال ايدي المستخدم وعدد النقاط).\n"
        "• `/remove_points <id> <points>` - خصم رصيد من حساب مستخدم معين.\n"
        "• `/broadcast <الرسالة>` - إرسال إذاعة أو رسالة جماعية لجميع مستخدمي البوت في وقت واحد.\n"
        "• `/set_channel <@channel>` - تغيير القناة الإجبارية أو تحديث معرفها.\n"
        "• `/add_session <الرقم> <StringSession>` - إضافة رقم مع الجلسة الخاصة به."
    )
...
def stats_command(message):
    ...
    stats_text = (
        f"📊 **إحصائيات البوت الشاملة (للمالك فقط):**\n\n"
        f"• عدد المستخدمين الكلي: {len(users)}\n"
        f"• إجمالي النقاط المُوزعة: {round(total_balance, 4)} $\n"
        f"• عدد الأرقام/الحسابات المتاحة: {len(numbers)}"
    )
...
def add_points(message):
    ...
    try:
        ...
        if target_id in users:
            ...
            save_data(USERS_FILE, users)
            bot.reply_to(message, f"✅ تم إضافة {amount} نقطة لحساب `{target_id}`", parse_mode='Markdown')
            try:
                bot.send_message(target_id, f"🎉 تم إضافة {amount} نقطة إلى رصيدك بواسطة الإدارة!")
            except Exception:
                pass
        else:
...
def remove_points(message):
    ...
    try:
        ...
        users = load_data(USERS_FILE)
        if target_id in users:
            users[target_id]["balance"] = max(0.0, users[target_id]["balance"] - float(amount))
            save_data(USERS_FILE, users)
            bot.reply_to(message, f"✅ تم خصم {amount} نقطة من حساب `{target_id}`", parse_mode='Markdown')
        else:
            bot.reply_to(message, "❌ المستخدم غير موجود في قاعدة البيانات.")
    except Exception:
        bot.reply_to(message, "⚠️ الاستخدام الصحيح:\n`/remove_points 123456789 5`", parse_mode='Markdown')
...
def broadcast(message):
    ...
    try:
        ...
        bot.reply_to(message, "⚠️ اكتب الرسالة بعد الأمر، مثال:\n`/broadcast السلام عليكم`", parse_mode='Markdown')
...
def set_channel(message):
    ...
    if message.from_user.id != ADMIN_ID:
        return
    try:
        new_channel = message.text.split(maxsplit=1)[1]
        REQUIRED_CHANNEL = new_channel
        bot.reply_to(message, f"✅ تم تحديث القناة الإجبارية إلى: {REQUIRED_CHANNEL}")
    except IndexError:
        bot.reply_to(message, "⚠️ الاستخدام الصحيح:\n`/set_channel @KNAS1_BOT`", parse_mode='Markdown')
...
def add_session_cmd(message):
    ...
    try:
        ...
        bot.reply_to(message, "⚠️ الاستخدام الصحيح:\n`/add_session +56991606880 StringSession...`", parse_mode='Markdown')
