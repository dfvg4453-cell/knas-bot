...
AUTO_PRICE = 0.20  # سعر العشوائي 0.20$
...
# كيبورد الأزرار السفلية الثابتة
def get_reply_keyboard():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(
        types.KeyboardButton("🛒 شراء حسابات"),
        types.KeyboardButton("💳 شحن الرصيد"),
        types.KeyboardButton("🔄 تحويل الرصيد"),
        types.KeyboardButton("💬 الدعم"),
        types.KeyboardButton("🎁 مشاركة وتجميع رصيد"),
        types.KeyboardButton("📊 الإحصائيات")
    )
    return markup

# قائمة الشراء (مع زر AUTO بسعر 0.20$ والمخزون)
def get_buy_menu():
    markup = types.InlineKeyboardMarkup(row_width=1)
    
    # إضافة الأقسام بالترتيب العمودي
    ...
    # زر AUTO مع السعر 0.20$ والكمية
    total_stock = len(load_json(NUMBERS_FILE))
    auto_btn_text = f"AUTO | {AUTO_PRICE}$ | {total_stock}"
    markup.add(types.InlineKeyboardButton(auto_btn_text, callback_data="buy_auto"))
...
def start_cmd(message):
    ...
    bot.send_message(chat_id, text, reply_markup=get_reply_keyboard(), parse_mode="Markdown")
...
def handle_text_messages(message):
    ...
    if text == "🛒 شراء حسابات":
        bot.send_message(chat_id, "اختر الرقم الذي تريده من القائمة:", reply_markup=get_buy_menu())
    elif text == "💳 شحن الرصيد":
        bot.send_message(chat_id, "للتواصل لشحن الرصيد: @K5XYY", reply_markup=get_reply_keyboard())
    elif text == "🔄 تحويل الرصيد":
        bot.send_message(chat_id, "قسم تحويل الرصيد قيد الصيانة حالياً.", reply_markup=get_reply_keyboard())
    elif text == "💬 الدعم":
        bot.send_message(chat_id, "للتواصل مع الدعم: @K5XYY", reply_markup=get_reply_keyboard())
    elif text == "🎁 مشاركة وتجميع رصيد":
        bot.send_message(chat_id, f"رابط الدعوة الخاص بك:\nhttps://t.me/{bot.get_me().username}?start={user_id}", reply_markup=get_reply_keyboard())
    elif text == "📊 الإحصائيات":
        bot.send_message(chat_id, f"📊 إحصائيات حسابك:\n- الأيدي: `{user_id}`\n- الرصيد: **{balance:.1f} $**", parse_mode="Markdown", reply_markup=get_reply_keyboard())
    else:
        bot.send_message(chat_id, "أهلاً بك! استخدم الأزرار بالأسفل للتنقل.", reply_markup=get_reply_keyboard())
...
def callback_listener(call):
    ...
    if call.data == "buy_menu":
        ...
        try:
            ...
            pass
        bot.send_message(chat_id, "القائمة الرئيسية:", reply_markup=get_reply_keyboard())

    # زر AUTO (رقم عشوائي بسعر 0.20$ مع التحقق من الرصيد)
    elif call.data == "buy_auto":
        current_balance = users_data[user_id]["balance"]
        if current_balance < AUTO_PRICE:
            bot.answer_callback_query(call.id, f"❌ رصيدك غير كافٍ! سعر العشوائي هو {AUTO_PRICE}$", show_alert=True)
            return
        ...
        text = f"✅ **تم شراء الرقم بنجاح (عشوائي):**\n\n📱 الرقم: `+{phone}`\n💰 الخصم: **{AUTO_PRICE}$**"
