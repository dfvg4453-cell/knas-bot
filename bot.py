...
# إعدادات Pyrogram للجلسات (استبدل القيم بمعلوماتك من my.telegram.org)
API_ID = 123456  
...
def handle_callbacks(call):
    ...
    if call.data == "buy_accounts":
        ...
        if code:
            ...
            bot.send_message(call.message.chat.id, "⏳ لم يصل الكود بعد! حاول مجدداً بعد طلب الكود من التليجرام.")
...
def add_session_cmd(message):
    ...
    try:
        ...
        save_data(db)
        bot.reply_to(message, f"✅ تم إضافة الرقم `{phone}` لقسم `{country}` بنجاح!", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ حدث خطأ أثناء إضافة الجلسة:\n`{e}`", parse_mode="Markdown")
