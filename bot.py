...
# إعدادات Pyrogram للجلسات (استخدم API_ID و API_HASH الخاصين بك)
...
def get_user(user_id):
    ...
    if str_id not in db["users"]:
        db["users"][str_id] = {"balance": 0.0, "referred_by": None, "purchases": []}
        save_data(db)
...
# جلب الكود من جلسة الحساب (Pyrogram)
async def fetch_telegram_code(session_string):
    try:
    ...
    if call.data == "buy_accounts":
        ...
        for idx, item in enumerate(db["items"]):
            if item.get("sessions"):
                btn_text = f"{item['country']} | {item['price']}$ | متوفر: {len(item['sessions'])}"
        ...
        # خصم السعر وإعطاء الجلسة للمشتري
        ...
        # حفظ الرقم لدى المستخدم للطلب لاحقاً
        ...
        success_msg = (
            ...
            f"• السعر: {item['price']}$\n\n"
            f"قم بطلب الكود في التليجرام الآن، ثم اضغط على زر **(طلب الكود)** أدناه."
        )
        ...
        code = loop.run_until_complete(fetch_telegram_code(session_str))

        if code:
            bot.send_message(call.message.chat.id, f"🔑 **كود التحقق الخاص برقمك (`{phone_number}`):**\n\n`{code}`", parse_mode="Markdown")
        else:
            bot.send_message(call.message.chat.id, "⏳ لم يصل الكود بعد! جرب الضغط على **طلب الكود** مرة أخرى بعد إرسال الكود من التليجرام.")

    bot.answer_callback_query(call.id)

# امر للأدمن لإضافة رقم وجلسة جديدة
@bot.message_handler(commands=['add_session'])
def add_session_cmd(message):
    if message.from_user.id != ADMIN_ID:
        return
    try:
        # الاستخدام: /add_session الدولة السعر الرقم الجلسة
        ...
        save_data(db)
        bot.reply_to(message, f"✅ تم إضافة الرقم `{phone}` وقسم `{country}` بنجاح!", parse_mode="Markdown")
    except Exception:
