...
def get_user(user_id):
    ...
    if str_id not in db["users"]:
        db["users"][str_id] = {"balance": 0.0, "points": 0.0, "referred_by": None}
        save_data(db)
    return db["users"][str_id]

# القائمة الرئيسية المرتبة
def main_keyboard(user_id):
    ...
    btn_points = InlineKeyboardButton("تجميع النقاط 🎁", callback_data="points")
    ...
    if is_new_user:
        ...
        if len(text_args) > 1:
            ...
            if referrer_id != str_id and referrer_id in db["users"]:
                ...
                db["users"][referrer_id]["points"] = round(db["users"][referrer_id].get("points", 0.0) + 0.01, 4)
                try:
                    bot.send_message(int(referrer_id), "🎉 دخل شخص جديد عبر رابط الدعوة الخاص بك! تم إضافة 0.01 نقطة لرصيدك.")
                except Exception:
                    pass
        save_data(db)
...
def handle_callbacks(call):
    ...
    if call.data == "points":
        ...
        points_text = (
            ...
            f"شارك الرابط مع أصدقائك، ولكل شخص ينضم عبر رابطك ستحصل على **0.01** نقطة!"
        )
        ...
        try:
            ...
            if idx < len(items):
                ...
                if user["balance"] < item["price"]:
                    bot.answer_callback_query(call.id, "❌ رصيدك غير كافٍ لشراء هذه السلعة!", show_alert=True)
                    return
                # خصم المبلغ وإرسال تفاصيل السلعة
                user["balance"] = round(user["balance"] - item["price"], 2)
                save_data(db)
                bot.send_message(call.message.chat.id, f"✅ تم شراء السلعة بنجاح!\nالقسم: {item['country']}\nالسعر الخصم: {item['price']}$")
            else:
                bot.answer_callback_query(call.id, "❌ السلعة غير موجودة أو انتهت.", show_alert=True)
        except Exception:
            pass
...
@bot.message_handler(commands=['na'])
def add_points_cmd(message):
    ...
    try:
        ...
        if target_id not in db["users"]:
            db["users"][target_id] = {"balance": 0.0, "points": 0.0, "referred_by": None}

        db["users"][target_id]["points"] = round(db["users"][target_id].get("points", 0.0) + pts, 4)
        save_data(db)
        bot.reply_to(message, f"🎁 تم إضافة {pts} نقطة للمستخدم `{target_id}` بنجاح!", parse_mode="Markdown")
    except Exception:
        bot.reply_to(message, "⚠️ طريقة الاستخدام:\n`/na آيدي_الشخص عدد_النقاط`", parse_mode="Markdown")
