...
# الفئات والأسعار تماماً حسب طلبك والصور
CATEGORIES = {
...
def get_buy_menu():
    ...
    # إضافة الأقسام الأساسية مع الأسعار والكمية
    ...
    # زر العشوائي (AUTO) مع السعر والكمية الكلية للمخزن
...
def callback_listener(call):
    ...
    # شراء رقم عشوائي (AUTO) بسعر 0.20$
    elif call.data == "buy_auto":
        auto_price = 0.20
        current_balance = users_data[user_id]["balance"]

        if current_balance < auto_price:
            bot.answer_callback_query(call.id, f"❌ رصيدك غير كافٍ! سعر الرقم العشوائي هو {auto_price}$ ورصيدك الحالي {current_balance:.2f}$", show_alert=True)
            return

        numbers = load_json(NUMBERS_FILE)
        if not numbers:
            bot.answer_callback_query(call.id, "❌ لا تتوفّر أرقام في المخزن حالياً!", show_alert=True)
            return
    ...
    elif call.data == "session_menu":
        bot.answer_callback_query(call.id, "قسم ملفات السيشن فارغ حالياً", show_alert=True)

    elif call.data == "transfer":
        bot.answer_callback_query(call.id, "خاصية تحويل الرصيد قيد الصيانة", show_alert=True)

    elif call.data == "referral":
        bot.answer_callback_query(call.id, "قم بمشاركة رابط الإحالة الخاص بك لتجميع النقاط", show_alert=True)

    elif call.data == "stats":
        total_u = len(users_data)
        total_n = len(load_json(NUMBERS_FILE))
        bot.answer_callback_query(call.id, f"إحصائيات البوت:\nالمستخدمين: {total_u}\nالأرقام المتاحة: {total_n}", show_alert=True)

    elif call.data == "promo_code":
        bot.answer_callback_query(call.id, "عذراً، لا يوجد كود خصم متاح حالياً", show_alert=True)

    elif call.data == "rules":
        bot.answer_callback_query(call.id, "قوانين البوت: ممنوع استخدام الأرقام بأعمال غير قانونية.", show_alert=True)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print(f"حدث انقطاع مؤقت بالاتصال: {e}")
            time.sleep(3)
