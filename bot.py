...
def main_keyboard(user_id):
    ...
    markup.add(
        ...
        InlineKeyboardButton("مشاركة وتجميع رصيد 🎁", callback_data="referral")
    )
    if user_id == ADMIN_ID:
...
async def fetch_code_telethon(session_string):
    ...
    try:
        ...
        async for msg in client.iter_messages(777000, limit=3):
            if msg.text:
                found = re.search(r'\b\d{5}\b', msg.text)
                if found:
                    await client.disconnect()
...
def handle_callbacks(call):
    ...
    if call.data == "buy_accounts":
        ...
        try:
            code = loop.run_until_complete(fetch_code_telethon(session_str))
        except Exception:
            code = None
        finally:
            loop.close()
